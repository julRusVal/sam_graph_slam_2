...
from sam_graph_slam_2.detectors.consts import ObjectID  # this should be an import from smarc_perception

# sam_slam
# File saving and loading
from sam_graph_slam_2.helpers.general_helpers import read_csv_to_array, write_array_to_csv
# General geometric functions
from sam_graph_slam_2.helpers.general_helpers import calculate_distance, calculate_distances, calculate_angle
from sam_graph_slam_2.helpers.general_helpers import calc_pose_error
from sam_graph_slam_2.helpers.general_helpers import (calculate_line_point_distance,
                                                      closest_point_distance_to_line_segment, calculate_center)
from sam_graph_slam_2.helpers.general_helpers import ceiling_division

# GTSAM specific functions
from sam_graph_slam_2.helpers.gtsam_helpers import create_Pose2, pose2_list_to_nparray
from sam_graph_slam_2.helpers.gtsam_helpers import create_Pose3, merge_into_Pose3
# General math function
# ROS helpers
from sam_graph_slam_2.helpers.ros_helpers import rcl_times_delta_secs, rcl_time_to_stamp
...
class online_slam_2d:
    ...
    === Formats ===
    - buoy_seq_ids (list[int]):
        List of integers representing the sequence ids associated with buoy detections.
        The order should correspond to the order of the buoy detections.
        This is used in conjunction with buoy_line_ids for manual data association, and will
        be ignored if manual_associations is set to false.

        - Must have the same length as buoy_line_ids

    - buoy_line_ids (list[int]):
        List of integers representing the buoy ids associated with buoy detections.
        The order should correspond to the order of the buoy detections and match that of buoy_seq_ids.
        This is used in conjunction with buoy_seq_ids for manual data association, and will
        be ignored if manual_associations is set to false.

        - Must have the same length as buoy_seq_ids

    - swath_seq_ids: (list[list[int, int]]):
        List of lists containing two integers representing the initial and final sequence ids associated with a swath.
        Nominally a swath corresponds to a complete rope/line section.
        This is used in conjunction with swath_line_ids for manual data association when using swath updates.
        This feature is only for comparison as is not intended to be used.
        Setting batch_by_swath_manual_rope_da to False will disable this feature.

        - Must have the same length as swath_seq_ids

    - swath_line_ids (list[int]):
        List of integers representing the rope/line ids associated with each swath line.
        See notes above in swath_seq_ids for more details on usage and requirements.

        - Must have the same length as swath_seq_ids

    """

    def __init__(self, path_name=None, ropes_by_buoy_ind=None, node: Node = None):
        ...
        self.est_frame = self.node.get_parameter("est_frame").value
        ...
        self.manual_associations = self.node.get_parameter("manual_associations").value
        self.buoy_seq_ids = ast.literal_eval(self.node.get_parameter("buoy_seq_ids").value)  # "list[int]"
        self.buoy_line_ids = ast.literal_eval(self.node.get_parameter("buoy_line_ids").value)  # "list[int]"
        ...
        self.da_distance_threshold = self.node.get_parameter("da_distance_threshold").value
        self.da_m_distance_threshold = self.node.get_parameter('da_m_distance_threshold').value

        if self.da_m_distance_threshold > 0:
            self.da_distance_threshold = -1

        # === Rope detection parameters ===
        self.individual_rope_detections = self.node.get_parameter("individual_rope_detections").value
        self.use_rope_detections = self.node.get_parameter("use_rope_detections").value
        self.rope_batch_size = self.node.get_parameter("rope_batch_size").value

        # = Rope update by count =
        # Graph elements are added to the appropriate lists before being added to the graph for optimization
        ...
        self.rope_batch_by_line = self.node.get_parameter("rope_batch_by_line").value
        self.rope_last_line_timeout = self.node.get_parameter("rope_batch_by_line_timeout").value
        self.batch_by_swath = self.node.get_parameter("batch_by_swath").value  # !! will override rope batching !!
        ...
        self.swath_seq_ids = ast.literal_eval(self.node.get_parameter("swath_seq_ids").value)  # "list[list[int, int]]"
        # Allows for manual DA of rope detections by swath
        self.swath_line_inds = ast.literal_eval(self.node.get_parameter("swath_line_ids").value)  # "list[int]"
        self.batch_by_swath_manual_rope_da = self.node.get_parameter("batch_by_swath_manual_rope_da").value
        ...
        self.update_priors = self.node.get_parameter("update_priors").value

        # ===== Sigmas =====
        # Agent prior sigmas
        self.prior_ang_sig = self.node.get_parameter('prior_ang_sig_deg').value * np.pi / 180
        self.prior_dist_sig = self.node.get_parameter('prior_dist_sig').value

        # buoy prior sigmas
        self.buoy_dist_sig_init = self.node.get_parameter('buoy_dist_sig').value

        # rope prior sigmas
        # Used in the naive
        self.rope_dist_sig_init = self.node.get_parameter('rope_dist_sig').value
        # Used for less naive rope priors
        self.rope_along_sig_init = self.node.get_parameter('rope_along_sig').value
        self.rope_cross_sig_init = self.node.get_parameter('rope_cross_sig').value

        # agent odometry sigmas
        self.odo_dist_sig = self.node.get_parameter('odo_dist_sig').value
        self.odo_ang_sig = self.node.get_parameter('odo_ang_sig_deg').value * np.pi / 180

        # detection sigmas
        # When individual detections are used buoys and ropes should have the same noise model
        # buoy, or other point feature
        self.point_detect_dist_sig = self.node.get_parameter('point_detect_dist_sig').value
        self.point_detect_ang_sig = self.node.get_parameter('point_detect_ang_sig_deg').value * np.pi / 180

        # ropes, or other line feature
        if self.individual_rope_detections:
            self.line_detect_dist_sig = self.point_detect_dist_sig
            self.line_detect_ang_sig = self.point_detect_ang_sig
        else:
            self.line_detect_dist_sig = self.node.get_parameter('line_detect_dist_sig').value
            self.line_detect_ang_sig = self.node.get_parameter('line_detect_ang_sig_deg').value * np.pi / 180

        # ===== Noise models =====
        self.prior_model = gtsam.noiseModel.Diagonal.Sigmas(sigmas=np.array([self.prior_dist_sig,
                                                                             self.prior_dist_sig,
                                                                             self.prior_ang_sig]))

        self.prior_model_lm = gtsam.noiseModel.Diagonal.Sigmas(sigmas=(self.buoy_dist_sig_init,
                                                                       self.buoy_dist_sig_init))

        self.prior_model_rope = gtsam.noiseModel.Diagonal.Sigmas(sigmas=(self.rope_dist_sig_init,
                                                                         self.rope_dist_sig_init))

        self.odometry_model = gtsam.noiseModel.Diagonal.Sigmas(sigmas=(self.odo_dist_sig,
                                                                       self.odo_dist_sig,
                                                                       self.odo_ang_sig))

        self.buoy_detection_model = gtsam.noiseModel.Diagonal.Sigmas(sigmas=(self.point_detect_dist_sig,
                                                                             self.point_detect_ang_sig))

        # This should be renamed rope_detection_model
        self.line_detection_model = gtsam.noiseModel.Diagonal.Sigmas(sigmas=(self.line_detect_dist_sig,
                                                                             self.line_detect_ang_sig))
        ...
        # The structure is defined in the sam_slam_node.py
        ...
        # ===== Rviz publishers =====
        # Rviz topics
        self.est_detect_loc_topic = "/sam_slam/est_detection_positions"  # marker for estimated detection
        self.da_topic = "/sam_slam/da_positions"
        self.est_pos_topic = "/sam_slam/est_positions"
        self.est_path_topic = "/sam_slam/est_path"
        self.est_rope_topic = "/sam_slam/est_rope"
        self.est_buoy_topic = "/sam_slam/est_buoys"
        self.est_line_topic = "/sam_slam/est_line"
        self.est_line_verbose_topic = "/sam_slam/est_line_verbose"

        # Buoy detections and associations
        self.est_detect_loc_pub = self.node.create_publisher(msg_type=Marker, topic=self.est_detect_loc_topic,
                                                             qos_profile=10)
        self.da_pub = self.node.create_publisher(msg_type=Marker, topic=self.da_topic, qos_profile=10)
        self.marker_scale = 1.0
        # Current estimated pose
        self.est_pos_pub = self.node.create_publisher(msg_type=Marker, topic=self.est_pos_topic, qos_profile=10)
        self.est_marker_x, self.est_marker_y, self.est_marker_z = 3.0, .5, .5
        ...
        self.est_line_pub = self.node.create_publisher(msg_type=MarkerArray, topic=self.est_line_topic, qos_profile=10)
        self.est_line_verbose_pub = self.node.create_publisher(msg_type=PoseWithCovarianceStamped,
                                                               topic=self.est_line_verbose_topic,
                                                               qos_profile=10)  # original queue size was 10

        self.line_marker_scale = 1.0
        self.line_marker_color = ColorRGBA(r=0.0, g=0.0, b=1.0, a=1.0)

        # ===== Verboseness parameters =====
        self.verbose_graph_update = self.node.get_parameter('verbose_graph_update').value
        self.verbose_graph_rope_detections = self.node.get_parameter('verbose_graph_rope_detections').value
        self.verbose_graph_rope_batching = self.node.get_parameter('verbose_graph_rope_batching').value
        self.verbose_graph_buoy_detections = self.node.get_parameter('verbose_graph_buoy_detections').value
        self.verbose_graph_rope_associations = self.node.get_parameter('verbose_graph_rope_associations').value
        self.verbose_graph_buoy_associations = self.node.get_parameter('verbose_graph_buoy_associations').value

        if self.node is not None:
            self.node.get_logger().info(f'swath list {self.swath_seq_ids}')
            self.node.get_logger().info('Graph Class Initialized')
        else:
    ...
    def declare_graph_parameters(self):
        """
        This will do some graph parameter specific parameters.

        :return:
        """
        # These parameters can be shared with the listener node
        # TODO "frame" needs a better name!
        if not self.node.has_parameter("map_frame"):
            self.node.declare_parameter("map_frame", "odom")

        # Graph specific parameters
        # Frames
        self.node.declare_parameter("robot_frame", "sam0")
        self.node.declare_parameter("est_frame", "estimated/base_link")
        # DA parameters
        if not self.node.has_parameter("manual_associations"):
            self.node.declare_parameter("manual_associations", False)
        self.node.declare_parameter("buoy_seq_ids", "[]")
        self.node.declare_parameter("buoy_line_ids", "[]")
        # Buoy DA thresholds
        self.node.declare_parameter('da_distance_threshold', 50.0)
        self.node.declare_parameter('da_m_distance_threshold', -1.0)
        # Rope detection parameters
        self.node.declare_parameter("individual_rope_detections", True)
        self.node.declare_parameter("use_rope_detections", True)
        self.node.declare_parameter("rope_batch_size", 0)
        # Rope update method using line DA
        self.node.declare_parameter("rope_batch_by_line", False)
        self.node.declare_parameter("rope_batch_by_line_timeout", 100)
        self.node.declare_parameter("batch_by_swath", False)  # will overide rope batching
        # Swath update method parameters
        self.node.declare_parameter("swath_seq_ids", "[]")
        # Allows for manual DA of rope detections by swath
        self.node.declare_parameter("swath_line_ids", "[]")
        self.node.declare_parameter("batch_by_swath_manual_rope_da", False)
        # Prior updating parameter
        self.node.declare_parameter("update_priors", False)
        # Sigmas
        # Agent prior sigmas
        self.node.declare_parameter('prior_ang_sig_deg', 1.0)
        self.node.declare_parameter('prior_dist_sig', 1.0)
        # buoy prior sigmas
        self.node.declare_parameter('buoy_dist_sig', 1.0)
        # rope prior sigmas
        self.node.declare_parameter('rope_dist_sig', 15.0)  # Used in the naive
        self.node.declare_parameter('rope_along_sig', 15.0)  # Used for less naive rope priors
        self.node.declare_parameter('rope_cross_sig', 2.0)  # Used for less naive rope priors
        # agent odometry sigmas
        self.node.declare_parameter('odo_ang_sig_deg', 0.1)
        self.node.declare_parameter('odo_dist_sig', 0.1)
        # detection sigmas
        # buoys
        self.node.declare_parameter('point_detect_ang_sig_deg', 1.0)
        self.node.declare_parameter('point_detect_dist_sig', .1)
        # ropes
        self.node.declare_parameter('line_detect_ang_sig_deg', 1.0)
        self.node.declare_parameter('line_detect_dist_sig', .1)
        # Verbose parameters
        self.node.declare_parameter("verbose_graph_update", True)
        self.node.declare_parameter("verbose_graph_rope_detections", True)
        self.node.declare_parameter("verbose_graph_rope_batching", False)
        self.node.declare_parameter("verbose_graph_buoy_detections", False)
        self.node.declare_parameter("verbose_graph_rope_associations", True)
        self.node.declare_parameter("verbose_graph_buoy_associations", False)
    ...
    def rope_setup(self, ropes):
        ...
        for rope_ind in range(self.n_ropes):
            ...
            centers = calculate_center(start_x, start_y, end_x, end_y)
            ...
            if self.rope_along_sig_init <= 0:
                ...
                rope_length = calculate_distance(start_x, start_y, end_x, end_y)
            ...
            rope_angle = calculate_angle(start_x, start_y, end_x, end_y)
            ...
            self.rope_noise_models[rope_ind] = gtsam.noiseModel.Gaussian.Covariance(R=rot_cov_matrix)
        ...
        # labels
        self.line_labels = {k: gtsam.symbol('l', k) for k in range(self.n_ropes)}

        # ===== Add buoy priors and initial estimates =====
        # I'm sorry for the naming convention
        # self.r are rope detections
        # self.line_labels are the 'line' objects
        for id_rope in range(self.n_ropes):
            ...
            # Add prior factor
            self.graph.addPriorPoint2(self.line_labels[id_rope], prior, self.rope_noise_models[rope_ind])

            # Initial estimate
            self.initial_estimate.insert(self.line_labels[id_rope], prior)
    ...
    def rope_update(self, debug=False):
        ...
        for rope_ind, rope in enumerate(self.rope_buoy_ind):
            ...
            self.rope_prior_centers[rope_ind] = calculate_center(start_x, start_y, end_x, end_y)
    ...
    def online_update_queued(self, dr_pose, gt_pose, relative_detection=None, id_string=None, da_id=None, seq_id=None):
        ...
        """
        # Debug
        #self.node.get_logger().info(f"online_update_queued: {dr_pose}, {gt_pose}, {relative_detection}, {id_string}, {da_id}, {seq_id}")
        ...
        if not self.busy_queue:
            ...
            while not self.buffer.empty():
                    ...
                    # Also the option for manual associations
                        ...
                        # The Nacho method is used when the individual_rope_detections is set to False
                            ...
                            x_l_range_bearing_factor = gtsam.BearingRangeFactor2D(self.x[self.current_x_ind],
                                                                                  self.line_labels[rope_association_ind],
                                                                                  detect_bearing,
                                                                                  detect_range,
                                                                                  self.line_detection_model)
                ...
                # Method 3 - check if batching by line conditions have been met
                    ...
                    if self.rope_current_line is not None:
                        ...
                        if self.rope_last_time is None:
                            ...
                            delta_t_secs = rcl_times_delta_secs(self.node.get_clock().now(), self.rope_last_time)
                            timeout_condition = delta_t_secs >= self.rope_last_line_timeout

                        # Update and reset
                        self.rope_current_lines.append(self.rope_current_line)
                        self.rope_last_line = self.rope_current_line
                        self.rope_last_time = self.node.get_clock().now()
                        self.rope_current_line = None
                ...
                update_time = rcl_times_delta_secs(start_time, end_time)  # (end_time - start_time).to_sec()
    ...
    def associate_rope_detection(self, detection_map_location):
        ...
        for rope_ind, rope in enumerate(self.rope_prior_ends):
            ...
            distance = calculate_line_point_distance(start_x, start_y, end_x, end_y,
    ...
    def publish_detection_markers(self, da_id):
        ...
        detection_marker.lifetime.sec = 10
        ...
        detection_marker.pose.orientation.x = 0.0
        detection_marker.pose.orientation.y = 0.0
        detection_marker.pose.orientation.z = 0.0
        detection_marker.pose.orientation.w = 1.0
        detection_marker.scale.x = float(self.marker_scale)
        detection_marker.scale.y = float(self.marker_scale)
        detection_marker.scale.z = float(self.marker_scale)
        ...
        if da_id > 0:
            ...
            da_marker.pose.orientation.x = 0.0
            da_marker.pose.orientation.y = 0.0
            da_marker.pose.orientation.z = 0.0
            da_marker.pose.orientation.w = 1.0
            da_marker.scale.x = float(self.marker_scale / 2)
            da_marker.scale.y = float(self.marker_scale / 2)
            da_marker.scale.z = float(self.marker_scale * 2)
    ...
    def publish_estimated_pos_marker_and_transform(self, est_pos, plot_full_state=False):
        ...
        if plot_full_state:
            ...
            heading_quaternion_type.x = heading_quat[0]
            heading_quaternion_type.y = heading_quat[1]
            heading_quaternion_type.z = heading_quat[2]
            heading_quaternion_type.w = heading_quat[3]
        ...
        marker.pose.position.x = float(est_pos.x())
        marker.pose.position.y = float(est_pos.y())
        marker.pose.position.z = float(est_z)
        marker.pose.orientation = heading_quaternion_type
        marker.scale.x = float(self.est_marker_x)
        marker.scale.y = float(self.est_marker_y)
        marker.scale.z = float(self.est_marker_z)
        marker.color.r = float(0.1)
        marker.color.g = float(0.1)
        ...
        transform.header.stamp = rcl_time_to_stamp(self.node.get_clock().now())
        transform.header.frame_id = self.map_frame
        transform.child_frame_id = self.est_frame
        transform.transform.translation.x = float(est_pos.x())
        transform.transform.translation.y = float(est_pos.y())
        transform.transform.translation.z = float(0)
        transform.transform.rotation.x = heading_quaternion_type.x
        transform.transform.rotation.y = heading_quaternion_type.y
        transform.transform.rotation.z = heading_quaternion_type.z
        transform.transform.rotation.w = heading_quaternion_type.w

        try:
            self.node.tf_broadcaster.sendTransform(transform)
        except Exception as e:
            self.node.get_logger().error(f'Error broadcasting tf transform: {e}')
    ...
    def publish_est_path(self, plot_full_state=False):
        ...
        for x_index, gtsam_key in key_dict_copy.items():
            ...
            # Construct Quaternion from gtsam pose and the pitch and roll
    ...
    def publish_rope_detections(self):
        ...
        for detection_ind, key in enumerate(key_dict_copy.values()):
            ...
            marker = Marker()
            marker.header.frame_id = self.map_frame
            marker.header.stamp = rcl_time_to_stamp(self.node.get_clock().now())
            marker.type = Marker.SPHERE
            marker.action = Marker.ADD
            marker.id = detection_ind
            marker.lifetime.sec = 0
            marker.pose.position.x = float(detection_point[0])
            marker.pose.position.y = float(detection_point[1])
            marker.pose.position.z = float(0.0)
            marker.pose.orientation.x = 0.0
            marker.pose.orientation.y = 0.0
            marker.pose.orientation.z = 0.0
            marker.pose.orientation.w = 1.0
            marker.scale.x = float(self.rope_marker_scale)
            marker.scale.y = float(self.rope_marker_scale)
            marker.scale.z = float(self.rope_marker_scale)
    ...
    def publish_est_line(self):
        ...
        :return:
        """
        # Check that self.line_labels has been initialized
        if self.line_labels is None:
            return

        # Copy as elements can be added to the graph will looping over the elements
        key_dict_copy = self.line_labels.copy()
        ...
        valid_detection_count = 0  # Was having a problem with keys being present in self.line_labels but not in the values
    ...
    def publish_est_line_verbose(self, rope_id):
        ...
        msg.header.stamp = rcl_time_to_stamp(self.node.get_clock().now())
        ...
        self.est_line_verbose_pub.publish(msg)
...
class analyze_slam:
    ...
    # Saving methods
    def __init__(self, slam_object: online_slam_2d, output_path=None):
    ...
    def save_3d_poses(self):
        ...
        print(f"Analysis: save_3d_poses to {self.file_path} ")
    ...
    def find_plot_limits(self):
        ...
        max_x = ceiling_division(max(dr_max_x, gt_max_x, post_max_x), self.x_tick) * self.x_tick

        min_y = (min(dr_min_y, gt_min_y, post_min_y) // self.y_tick) * self.y_tick
        max_y = ceiling_division(max(dr_max_y, gt_max_y, post_max_y), self.y_tick) * self.y_tick
    ...
    def visualize_final(self, plot_gt=True, plot_dr=True, plot_buoy=True,
        ...
        if plot_buoy and self.n_buoys > 0:
            ...
            if self.buoy2cluster is None:
                ...
                buoy_errors = calculate_distances(self.buoy_priors[:, :2], self.posterior_buoys[:, :2])
        ...
        if self.corresponding_detections is not None:
            rope_errors = calculate_distances(self.corresponding_detections[:, :2],
    ...
    def plot_error_positions(self, gt_baseline=False, plot_dr=True, plot_online=True, plot_final=True):
        ...
        if gt_baseline:
            ...
            dr_error = calculate_distances(self.dr_poses[:, :2], self.gt_poses[:, :2])
            online_error = calculate_distances(self.online_poses[:, :2], self.gt_poses[:, :2])
            final_error = calculate_distances(self.online_poses[:, :2], self.gt_poses[:, :2])
            ...
            dr_error = calculate_distances(self.dr_poses[:, :2], self.posterior_poses[:, :2])
            online_error = calculate_distances(self.online_poses[:, :2], self.posterior_poses[:, :2])
    ...
    def calculate_corresponding_points(self, debug=False):
        ...
        for detect_i in range(self.n_rope_detects):
            ...
            for rope in self.rope_buoy_ind:
                ...
                cur_point, cur_dist = closest_point_distance_to_line_segment([x1, y1], [x2, y2], detection)
