...
def find_centroid(points: np.ndarray):
    """
...
def project_3d_points_onto_plane(points: np.ndarray, vector_a, vector_b=None, point_on_plane=None):
...
def plot_2d_points(points: np.ndarray):
    fig = plt.figure()
    ax = fig.add_subplot()
...
def find_non_zero_indices(array: np.ndarray) -> Tuple[int, int, int, int]:
    """
...
class ProcessPointCloud2(object):
    def __init__(self, points_3d: np.ndarray, plot_process_results: bool = False):
        """
        Assumes points are given as Nx3 numpy array.
        """
        # Original 3d points
