...
def concat_arrays_with_time_threshold(orig_array: np.ndarray,
    ...
    :param orig_array: nx3 array, times at orig_array[:, 1]
    :param new_array: mx3 array, times at new_array[:, 1]
    ...
    filtered_new_array = np.array(
        [row for row in new_array if all(abs(row[time_index] - orig_array[:, time_index]) > time_threshold)])
...
def safe_vstack(array1, array2):
    """
    Concatenate two arrays. Will return None if both arrays are None
    :param array1:
    :param array2:
    :return:
    """
...
class process_sss:
    def __init__(self, sss_data: np.ndarray,
        ...
        sss_data: numpy array of sss return
        sss_times: numpy array of sss times
        """
        ...
        # Data
        self.img_original = sss_data
        self.time_ids = sss_times
        self.data_label = self.img_original.shape[0]
        ...
        if cpd_ratio is None:
            ...
            self.detector = CPDDetector(min_mean_diff_ratio=self.cpd_ratio)
    ...
    def flip_data(self, flipped_sections=None, show=False, overwrite_orig=False):
        ...
        for section in flipped_sections:
            ...
            seq_id_starts.append(int(self.time_ids[flip_start_ind]))
            seq_id_ends.append(int(self.time_ids[flip_end_ind - 1]))
    ...
    def filter_threshold(self, threshold=128, show=False):
        ...
        output_file_path = os.path.join(self.output_path, f'{self.data_label}_thresh.png')
    ...
    def down_range_gradient(self, m_size=5, g_size=5, s_size=5, show=True, save=False):
        ...
        if s_size in [3, 5, 7, 9]:
            ...
            dx_port_neg[dx_port_neg > 0] = 0  # remove positive gradients
    ...
    #     output_file_path = os.path.join(self.output_path, f'combined_canny_grad.png')
    #     cv.imwrite(output_file_path, img_combined)
    ...
    def cpd_perform_detection(self, side=0):
        ...
        for i, ping in enumerate(img_side):
            ...
            if self.cpd_max_depth > 0:
                # TODO not sure where detect_rope_buoy is, maybe i didnt commit from the HUGE laptop
    ...
    def mark_manual_detections(self, manual_detections=None):
        ...
        output_file_path = os.path.join(self.output_path, f'{self.data_label}_marked.jpg')
        cv.imwrite(output_file_path, color_img)

    def extract_seq_ids(self, detections):
        if self.detections is None or self.time_ids is None:
            return
        self.buoy_seq_ids = []
        for detection in detections:
            self.buoy_seq_ids.append(self.time_ids[detection[0]])
    ...
    def show_thresholds(self, data, l_threshold, h_threshold, data_label='NO LABEL', reference_data=False):
        ...
        thresholding_fig.suptitle(
            f'Thresholding of {data_label} - L threshold: {l_threshold}  H threshold: {h_threshold}')
    ...
    def find_rising_edges(self, data, threshold, max_count=2, show=False, save_output=False):
        ...
        if save_output:
            # Port
            output_file_path = os.path.join(self.output_path, 'port_detection_inds.csv')
            np.savetxt(output_file_path, port_detection_inds, delimiter=",")
            # Starboard
            output_file_path = os.path.join(self.output_path, 'star_detection_inds.csv')
            np.savetxt(output_file_path, star_detection_inds, delimiter=",")
    ...
    def post_overlay_detections_pretty(self):
        ...
        image_path = os.path.join(self.output_path, f'detector_output_original_{start_ind}_{end_ind}.png')
        cv.imwrite(image_path, img_color[start_ind:end_ind, :end_ind_column, ::-1])

        image_path = os.path.join(self.output_path, f'detector_output_marked_{start_ind}_{end_ind}.png')
        cv.imwrite(image_path, img_combined[start_ind:end_ind, :end_ind_column, ::-1])
    ...
    def post_find_buoy_centers(self, min_region_size, exclusion_zone=None):
        ...
        # check for empty buoy detections
        if region_centers.shape[0] == 0:
            return None
        ...
        for i in range(region_centers.shape[0]):
            ...
            if port:
                ...
                test_1 = np.abs(y_coord - region_centers[:, 0]) < exclusion_zone  # proximity
                test_2 = region_centers[:, 1] >= self.post_width // 2  # correct region
                inds = np.where(test_1 & test_2)[0]
    ...
    def post_final_buoys(self, plot=False, save=False):
        ...
        # Determine the # of centers detected
        # If none are detected the size of self.post_buoy_centers should be 0 or None
        # If there are no detections self.final_buoys is None
        # TODO check how self.post_buoy_centers is set if no centers are detected
        if self.post_buoy_centers is None:
            buoy_count = 0
        else:
        ...
        self.final_bouys = np.zeros((buoy_count, 3), float)
        ...
        # find seq IDs
        relevant_time_ids = self.time_ids[self.final_bouys[:, 0].astype(int)]
    ...
    def post_final_ropes(self, plot=False, save=False):
        ...
        if len(port_valid_inds) > 0:
            port_seq_ids = self.time_ids[port_valid_inds]
        ...
        if len(star_valid_inds) > 0:
            star_seq_ids = self.time_ids[star_valid_inds]
    ...
    def perform_algae_farm_detection(self, show_output=False, save_output=False, verbose=False):
        ...
        limiting_max = 100
        limiting_show = False
        inds_med_size = 25  # 45 worked well
        ...
        sam_file_name = f'/home/julian/sss_data/sss_data_{data_set_length}.jpg'
        seq_file_name = f'/home/julian/sss_data/sss_seqs_{data_set_length}.csv'
        ...
        # ===== Canny edge detection for ropes =====
        # Custom in that only the down range gradients are used
        canny_custom, custom_dx, custom_dx_neg = self.canny_custom(canny_med_size,
                                                                   canny_gauss_size,
                                                                   canny_sobel_size,
        ...
        template_result = cv.matchTemplate(matching_image, template, cv.TM_CCOEFF)
        ...
        if show_template:
            self.show_thresholds(template_result,
                                 template_l_threshold,
                                 template_h_threshold,
                                 'Grad Template',
                                 reference_data=not show_template_raw and show_output)
        ...
        self.post_initialize(rope_detections=canny_custom,
                             buoy_detections=template_result_threshold,
                             use_port=True,
                             use_starboard=False)

        # Process buoy detections
        self.post_find_buoy_centers(min_region_size=buoy_center_size_threshold,  # 5
        ...
        self.post_remove_ringing_rope(max_count=ringing_max_count,
                                      show_results=ringing_show and show_output)

        # Enforce max detection range
        self.post_limit_range(min_index=limiting_min,
                              max_index=limiting_max,
                              show_results=limiting_show and show_output)

        self.post_exclude_rope_in_buoy_area(radius=rope_exclusion_size, show_results=False)

        post_port_detection_inds, post_star_detection_inds = self.find_rising_edges(
            data=self.post_rope,
            threshold=0,
            max_count=2,
            show=False,
        ...
        port_inter_raw, port_inter_med = self.post_interleave_detection_inds(
            channel_data=post_port_detection_inds,
            channel_id='port',
            med_filter_kernel=inds_med_size,
            show_results=save_output)

        star_inter_raw, star_inter_med = self.post_interleave_detection_inds(
            channel_data=post_star_detection_inds,
            channel_id='star',
            med_filter_kernel=inds_med_size,
            show_results=save_output)

        self.post_interleaved_to_2d(interleaved_port=port_inter_med,
                                    interleaved_star=star_inter_med)

        if show_final_inds_port and show_output:
            self.post_plot_inds(channel_id='port')
        ...
        if show_final_post and show_output:
            self.post_overlay_detections()
            self.post_overlay_detections_pretty()
...
if __name__ == "__main__":
    ...
    sss_analysis.perform_algae_farm_detection()
