...
class SamSlamListener(Node):
    ...
    def __init__(self):
        ...
        if len(self.pipeline_lines) > 0:  # If pipeline is defined
            self.ropes_by_buoy_ind = self.pipeline_lines

        elif self.simulated_data:  # Simulated algae farm
            # TODO move to launch
            # This needs to match the structure defined in simulation environment
            self.ropes_by_buoy_ind = [[0, 4], [4, 2],
                                      [1, 5], [5, 3]]
        ...
        # ===== Subscribers =====
        # Ground truth
        self.gt_subscriber = self.create_subscription(msg_type=Odometry, topic=self.gt_topic,
                                                      callback=self.gt_callback, qos_profile=10)

        # Dead reckoning
        self.dr_subscriber = self.create_subscription(msg_type=Odometry, topic=self.dr_topic,
        ...
        if self.pipeline_scenario:
            self.line_colors = self.get_parameter("pipeline_colors").value
        else:
    ...
    def declare_node_parameters(self):
        ...
        # Maybe scenario should be some string idea and the other values woudld be set accordingly
        self.declare_parameter("simulated_data", True)  # real or simulated data
        self.declare_parameter("simulated_detections", False)  # Maybe not used???
        self.declare_parameter("pipeline_scenario", True)  # indicates
        self.declare_parameter("record_ground_truth", True)  # indicates if GT is available
        # TODO consider what the default state of thing should be
